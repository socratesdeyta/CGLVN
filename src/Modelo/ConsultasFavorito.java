...
public class ConsultasFavorito extends Conexion {
...
//                dat.setNombre(rs.getString("seccion"));
    ...
    public boolean registrar(ModeloFavorito modFav, int subserieAsignada) {
        ...
        try {
            ...
            ps.setDate(2, modFav.getFecha_registro());
            ps.setInt(3, modFav.getSeccion());
            ps.setInt(4, modFav.getSerie());
            System.out.println("EN INSERTAR " + subserieAsignada);
            if (sub != 0) {
                ps.setInt(5, modFav.getSubserie());
                System.out.println("EN INSERTAR es diferente de 0 " + modFav.getSubserie());

            } else {
                ps.setNull(5, modFav.getSubserie());
                System.out.println("No existe Subserie " + modFav.getSubserie());
            }

            //ps.setNull(5, modMinSal.getSubserie());
            ps.setInt(6, modFav.getDestinatario());
            //ps.setString(7, modMinSal.getFuncionario());
            ps.setInt(7, modFav.getFuncionario());
            ps.setInt(8, modFav.getEnvio());
            ps.setString(9, modFav.getAsunto());
            ps.execute();
            return true;
        } catch (SQLException e) {
            System.err.println(e);
