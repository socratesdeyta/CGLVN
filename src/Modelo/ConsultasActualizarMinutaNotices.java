...
public class ConsultasActualizarMinutaNotices extends Conexion {
...
//    }
    ...
    public boolean Seleccionar(ModeloActualizarMinutaNotices mod, frmActualizarMinutaNotices frm) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection con = getConexion();
        try {
            int filaSeleccionada = frm.jtActMinNt.getSelectedRow();
            String clave = frm.jtActMinNt.getValueAt(filaSeleccionada, 0).toString();
            ps = con.prepareStatement("SELECT numero, fecha_registro, seccion, serie, subserie,"
                    + " destinatario, funcionario, envio, referencia, asunto FROM t_documentosenviados "
                    + "WHERE numero=?");
            ...
            while (rs.next()) {
                ...
                if (subserie == null) {
                    ...
                    System.out.println("Modelo.ConsultasActualizarMinutaNotices.Seleccionar()---->" + mod.getSubserie());
                }
    ...
    public boolean RegistrarFechaEnvio(ModeloActualizarMinutaNotices mod, Date fechaEnvio, int idFuncionario) {
        PreparedStatement ps = null;
        Connection con = getConexion();
...
//                + "id_tipodestino=?, id_subtipodestino=? WHERE numero_destino=?";
        try {
            ...
            System.err.println(e);
            System.err.println("Error el Actualizar Minuta Notice");
            return false;
        } finally {
            try {
                con.close();
...
//    } // fin de metodo eliminar
    ...
    public boolean buscarCaracter(ModeloActualizarMinutaNotices mod, frmActualizarMinutaNotices frm) {
...
//        jtActMinOfi.setDefaultRenderer(Object.class, formatoTabla);
        ...
        frm.jtActMinNt.setModel(modeloTabla);
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection con = getConexion();
...
//            int[] anchos = {10, 10, 30, 50};
            ...
            for (int i = 0; i < frm.jtActMinNt.getColumnCount(); i++) {
                frm.jtActMinNt.getColumnModel().getColumn(i).setPreferredWidth(anchos[i]);
            }
            while (rs.next()) {
...
//    }
    ...
    public boolean actualizarTabla(ModeloActualizarMinutaNotices mod, frmActualizarMinutaNotices frm) {
        DefaultTableModel modeloTabla = new DefaultTableModel();
        frm.jtActMinNt.setModel(modeloTabla);
        PreparedStatement ps = null;
        ResultSet rs = null;
...
//------------------------------------------------------------------------------------------
    ...
    //---------- Se puede ocupar uno solo para Notice y Oficios???????????????????
    
    public ArrayList<ModActNtArray> mostrarArregloDestino() {
        System.out.println("Entro a mostrar arreglo destino");
        ArrayList<ModActNtArray> datos = new ArrayList<>();
        ModActNtArray dat = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection con = getConexion();
